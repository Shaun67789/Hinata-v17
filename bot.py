...
async def cmd_couple(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.effective_message.reply_text("💞 <b>Finding the perfect couple...</b>", parse_mode="HTML")
    ...
    try:
        ...
        import time
        custom_seed = int(time.time() / 86400) + chat.id # Daily seeded couple
        ...
        async def get_pfp(user_id):
            try:
                ...
                if photos.total_count > 0:
                    ...
                    return Image.open(BytesIO(photo_bytes)).convert("RGBA")
            except: pass
            
            # Create a dummy colored box if no PFP
            img = Image.new('RGBA', (500, 500), color=(random.randint(50,200), random.randint(50,200), random.randint(50,200), 255))
            return img
        ...
        p1_img = p1_img.resize((500, 500))
        p2_img = p2_img.resize((500, 500))
        
        # Create Heart Overlay / Background
        bg_width, bg_height = 1100, 600
        bg = Image.new('RGBA', (bg_width, bg_height), color=(255, 240, 245, 255)) # Light pink BG
        
        # Paste PFPs
        bg.paste(p1_img, (20, 50))
        bg.paste(p2_img, (580, 50))
        
        # Add a heart emoji/text in the middle (using simple text drawing)
        draw = ImageDraw.Draw(bg)
        try:
             # Try standard fonts or default
             font = ImageFont.truetype("arial.ttf", 80)
        except:
             font = ImageFont.load_default()
             
        # Just drawing a simple "💖" cross might not render emojis perfectly in PIL, using text or simple shapes
        draw.text((515, 260), "+", fill=(255, 50, 100, 255), font=font)
        ...
        caption = (
            ...
            f"<i>\"A match made in heaven! May your bond grow stronger every day.\"</i> 🌹"
        )
        ...
        logger.error(f"Couple Command Error: {e}")
        await msg.edit_text("❌ System error while finding a couple.")
    if not await check_permission(update, context): return
    if len(context.args) < 2:
        await update.effective_message.reply_text(" <b>Usage:</b> <code>/love [person1] [person2]</code>", parse_mode="HTML")
        return
    ...
    msg = ""
    if percentage >= 90:
        msg = "💍 Soulmates! You two are destined for each other!"
    elif percentage >= 70:
        msg = "💖 Strong connection! Love is in the air."
    elif percentage >= 50:
        msg = "💕 There is a spark... let's see where it goes!"
    elif percentage >= 30:
        msg = "💔 Maybe stay friends for now..."
    else:
        msg = "💀 Absolutely no chemistry. Better luck next time."
        
    text = (
        f"💘 <b>Love Calculator</b> 💘\n\n"
        f"👤 {html.escape(p1)} + 👤 {html.escape(p2)}\n"
